...
const handleFollowersClick = () => {
    axios.get('connection/followers', { params: { visitedUserId: props.userResponse.user._id } }).then(res => {
        if (res && res.status === 201) {
            state.followers = res.data
        }
    }).then(res => {
        state.displayFollowers = true
    }).catch((err) => {})
}
const handleFollowingClick = () => {
    axios.get('connection/following', { params: { visitedUserId: props.userResponse.user._id } }).then(res => {
        if (res && res.status === 201) {
            state.following = res.data
        }
    }).then(res => {
        state.displayFollowing = true
    }).catch((err) => {})
}
onMounted(() => {
...
<template>
    <div class="wrapper max-w-[800px] ">
        ...
        <div class="d-wrapper">
            <div class="p-4">
                <div class="details relative group ">
                    <div :class="`profile-pic-container w-32 h-32 m-2  ${state.myProfile ? 'my-profile' : ''}`">
                        ...
                        <div v-if="userResponse.user._id === authStore._id" @click="toggleProfilePicEdit"
                            class="absolute flex justify-center cursor-pointer items-center h-32 w-32 bg-gray-200 invisible rounded-full opacity-40 group-hover:visible text-3xl">
                            <EditIcon />
                        </div>
                    </div>
            ...
            <div class="activity-container">
                <div v-if="authStore._id != userResponse.user._id">
                    <button v-if="state.followIsLoading" class="button follow-button bg-gray-400"
                        disabled>Follow</button>
                    <div v-else>
                        <button v-if="!state.followIsLoading && !state.followedByLoggedUser" @click="handleFollowClick"
                ...
                <div class="flex">
                    ...
                    <div class="flex text-lg cursor-pointer" @click="handleFollowersClick">
                        <div class="pr-0.5">{{ userResponse.followersCount }}</div>
                        <div> Followers</div>
                    </div>
                    <div class="flex text-lg cursor-pointer" @click="handleFollowingClick">
        ...
        <ChatSearchView v-if="state.displayFollowers" :isProfile="true"
            :handleToggleChatSearch="() => state.displayFollowers = false" :usersList="state.followers" />
        <ChatSearchView v-if="state.displayFollowing" :isProfile="true"
            :handleToggleChatSearch="() => state.displayFollowing = false" :usersList="state.following" />
    </div>
</template>
<style scoped>
.wrapper {
