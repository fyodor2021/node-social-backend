        ...
        <div class="c-container">
            <div class="side-wrapper">
                ...
                <div class="chat-user-list md:w-full">
                    <div v-for="user in state.usersList"
                        :class="`${state.selectedUser._id === user._id ? 'bg-black' : ''}`">
                        <ContentUser :isChatView="true" @click="handleSelectUser(user)" v-if="state.usersList"
                            :user="user" :signedProfilePic="user.signedProfilePic" :key="user._id"
                            :newMessage="socketStore.newMessageAlertSet.has(user._id)" />
                    </div>
                </div>
            </div>
            <div class="chat-box">
