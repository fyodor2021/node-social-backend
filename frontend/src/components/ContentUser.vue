...
<template>
    <div  @click="handleUserClick" :class="`flex items-center  justify-between cursor-pointer p-2 ${selected ? 'bg-black' : ''}`">
        <div v-if="user" class="flex items-start cursor-pointer ">
            <div class="profile-pic-container">
                <img v-if="signedProfilePic" 
                :src="signedProfilePic ? signedProfilePic : profile" :alt="user.fname" rel="preload" />
                <img v-else :src="profile" alt="sara" class="w-20" rel="preload" />
            </div>
            <div>
                <h1 :class="`font-bold ${selected ? 'text-white' : ''}`"> {{ capName(user.fname) }} {{ capName(user.lname) }}</h1>
                <h2 :class="`text-gray-700 ${selected ? 'text-gray-600' : ''}`">{{ user.tag ? '@' + user.tag : '' }}</h2>
