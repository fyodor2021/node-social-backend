...
<template>
    <div @click="handleUserClick"
        ...
        <div v-if="user" :class="`flex h-full items-center cursor-pointer justify-center px-1`">
            ...
            <div v-if="!hideName" :class="`relative ${isComment && 'text-[.75rem]' }  ${ isPost && 'h-full flex flex-col justify-start items-start' }`">
                ...
                <span v-if="isOnline" class="absolute top-[17px] right-[-20px] z-[99] w-2 h-2 bg-green-500 rounded-full m-4"></span>
                <span v-if="newMessage && !selected"  class="absolute top-[17px] right-[-20px] z-[99] w-2 h-2 bg-orange-500 rounded-full m-4"></span>
            </div>
        </div>
        <div @click="() => router.push({ name: 'userDetails', params: { userId: authStore._id } })" v-else
            class="flex items-center">
            <div :class="`profile-pic-container ${hideName && 'm-0 p-0'}`">
                <img :src="authStore.signedProfilePic ? authStore.signedProfilePic : profile" alt="sara" />
            </div>
            <h1 v-if="(authStore.fname || authStore.lname) && !hideName" class="font-bold"> {{ capName(authStore.fname) }} {{
