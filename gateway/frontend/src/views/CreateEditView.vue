...
const state = reactive({
    ...
    displayEmo: false,
    input: props.postResponse && props.postResponse.strContent ? props.isShare ? '' : props.postResponse.strContent : '',
    inputLetterCount: 0,
    valMessage: ''
})
...
const handleSubmitPost = (e) => {
    ...
    const postRequest = {
        ...
        postId: props.postResponse ? props.postResponse._id : '',
    }
    ...
    if (props.postResponse) {
        if (props.isShare) {
            const postRequest = {
                ...
                strContent: state.input,
                objContent: {
                    _id: props.postResponse._id,
                    user: props.postResponse.user,
                    strContent: props.postResponse.strContent,
                    fileName: props.postResponse.fileName,
                    objContent: props.postResponse.objContent ?
                        props.postResponse.objContent : ''
                },
            }
        ...
        if(!state.uploadedImage && state.input === props.postResponse.strContent){
            props.toggleFunction()
        }else{
    ...
    <div class="c-e-container text-gray-800 border border-gray-300 p-4 shadow-lg ">
        <form @submit.prevent="handleSubmitPost" enctype="multipart/form-data" class="wrapper" ref="element"
                ...
                <div class="relative">
                    <div class="upload-container">
                        ...
                        <div v-click-outside="() => state.displayEmo = false">
                            <i @click="toggleDisplayEmoji" :native="true"
                                :class="`pi pi-face-smile text-3xl`">
                            </i>
                            <EmojiPicker :native="true" class="absolute right-0 z-[999]" v-if="state.displayEmo" @select="onSelectEmoji"/>
                        </div>
                    </div>
            ...
            <div class="flex flex-col justify-space-between">
                ...
                <div v-if="(postResponse && isShare) || postResponse && postResponse.objContent">
                ...
                <div v-else class="w-full flex justify-center items-center p-4 relative group">
                    <img v-if="state.uploadedImage || (postResponse && !isShare)"
                        :src="state.uploadedImage ? state.uploadedImage : postResponse.signedPostPic"
                        class="w-full h-full" />
