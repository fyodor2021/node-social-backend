    ...
    if (sender && receiver) {
      ...
      try {
        if(typeof(message.content) === 'object'){
          const userFilename = await userModel.findOne({_id: message.content.user._id}).select('profilePic')
          message.content.user.signedProfilePic = userFilename.profilePic ?  userFilename.profilePic : ''
        }
        message.save();
        res.sendStatus(201);
      } catch (err) {
  ...
  const chats = await messageModel
    ...
    for(let chat of chats){
      ...
      if(typeof(chat.content) === 'object'){
        chat.content.user.signedProfilePic = chat.content.user.signedProfilePic ?
        await getSignedURL(chat.content.user.signedProfilePic) : ''
        chat.content.signedPostPic = chat.content.fileName ? 
        await getSignedURL(chat.content.fileName) : ''
      }
    }
