...
import multerGoogle from "multer-google-storage";
import { getSignedURL, deleteImage } from "../functions/gcsFunctions.js";
  ...
  if (
    ...
    if (post) {
      const postRequest = new postModel({
        user: req.body.user,
        strContent: req.body.strContent ? req.body.strContent : "",
...
async function preparePostResponse(post, userId) {
  ...
  if (postUser) {
    ...
    post.objContent = post.objContent
      ? await preparePostResponse(post.objContent)
      : "";
    const postResponse = {
      _id: post._id,
      date: post.date,
